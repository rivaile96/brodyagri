...
export default function GuideDetailPage() {
  ...
  return (
    <AppShell title={guide.title} rightSlot={
      <Link href="/guide" className="w-9 h-9 rounded-full bg-[#14231b] border border-emerald-800/40 flex items-center justify-center text-emerald-300 hover:text-white transition-colors">
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-3xl bg-[#14231b] border border-emerald-700/40 p-6 space-y-4 backdrop-blur-md shadow-sm">
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] font-bold text-amber-300 bg-amber-950/50 px-2.5 py-0.5 rounded-full border border-amber-800/40">
                {guide.difficulty}
              </span>
            </div>
          ...
          {/* Key Metrics Grid */}
          <div className="grid grid-cols-2 gap-2.5 pt-2 text-xs border-t border-emerald-800/60">
            <div className="bg-[#0c1410]/70 p-3 rounded-2xl border border-emerald-900/60 flex items-center gap-2.5">
              <Clock className="w-4 h-4 text-amber-400 shrink-0" />
              <div>
                <span className="text-[9px] text-emerald-400/80 font-mono block">Estimasi Panen</span>
                <strong className="text-white text-[11px] block">{guide.harvest_time}</strong>
              </div>
            </div>
            ...
            <div className="bg-[#0c1410]/70 p-3 rounded-2xl border border-emerald-900/60 flex items-center gap-2.5">
              ...
              <div>
                ...
                <strong className="text-white text-[11px] block">{guide.sunlight_req}</strong>
              </div>
            </div>
            ...
            <div className="bg-[#0c1410]/70 p-3 rounded-2xl border border-emerald-900/60 flex items-center gap-2.5">
              ...
              <div>
                ...
                <strong className="text-white text-[11px] block">{guide.optimal_temp}</strong>
              </div>
            </div>
            ...
            <div className="bg-[#0c1410]/70 p-3 rounded-2xl border border-emerald-900/60 flex items-center gap-2.5">
              ...
              <div>
                ...
                <strong className="text-white text-[11px] block">{guide.optimal_elevation}</strong>
              </div>
            </div>
          </div>
        ...
        <div className="flex bg-[#14231b]/90 border border-emerald-800/40 p-1.5 rounded-3xl backdrop-blur-md shadow-md text-xs font-bold">
          <button
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'overview' && (
            <motion.div
              ...
              className="space-y-4"
            >
              <div className="rounded-3xl bg-[#14231b]/90 border border-emerald-800/40 p-5 space-y-4 shadow-md">
                <div className="flex items-center gap-2 border-b border-emerald-900/80 pb-3">
                  <Shovel className="w-5 h-5 text-emerald-400" />
                  <h2 className="text-sm font-black text-white uppercase tracking-wider">Persiapan Pot &amp; Media Ideal</h2>
                </div>

                {media.pot_size && (
                  <div className="p-3 rounded-2xl bg-[#0c1410] border border-emerald-900/60 text-xs">
                    <span className="text-[10px] font-mono uppercase text-emerald-400 font-bold block mb-1">Rekomendasi Wadah:</span>
                    <strong className="text-white">{media.pot_size}</strong>
                  </div>
                )}
                ...
                {media.formula && (
                  <div className="p-3 rounded-2xl bg-[#0c1410] border border-emerald-900/60 text-xs">
                    ...
                    <strong className="text-white leading-relaxed block">{media.formula}</strong>
                    {media.ph_target && (
                      <span className="text-[10px] text-emerald-300/70 block mt-1">Target pH Tanah: {media.ph_target}</span>
                    )}
                  </div>
                )}
              ...
              {/* Tips Pro */}
              {tips.length > 0 && (
                <div className="rounded-3xl bg-[#14231b]/90 border border-emerald-800/40 p-5 space-y-3 shadow-md">
            ...
            >
              <div className="rounded-3xl bg-[#14231b]/90 border border-emerald-800/40 p-5 space-y-4 shadow-md">
                <div className="flex items-center gap-2 border-b border-emerald-900/80 pb-3">
                  ...
                  <h2 className="text-sm font-black text-white uppercase tracking-wider">Fase Vegetatif &amp; Bentuk Percabangan</h2>
                </div>

                <div className="space-y-3">
                  {vegSteps.map((step: any, idx: number) => (
                    <div key={idx} className="p-4 rounded-2xl bg-[#0c1410] border border-emerald-900/60 space-y-1.5">
                      ...
                      <p className="text-xs text-emerald-100/90 leading-relaxed pl-8">
                        {step.desc}
                      </p>
                    </div>
                  ))}
            ...
            >
              <div className="rounded-3xl bg-[#14231b]/90 border border-emerald-800/40 p-5 space-y-4 shadow-md">
                <div className="flex items-center gap-2 border-b border-emerald-900/80 pb-3">
                  ...
                  <h2 className="text-sm font-black text-white uppercase tracking-wider">Fase Generatif &amp; Jurus Pembuahan</h2>
                </div>
            ...
            >
              <div className="rounded-3xl bg-[#14231b]/90 border border-emerald-800/40 p-5 space-y-4 shadow-md">
                <div className="flex items-center gap-2 border-b border-emerald-900/80 pb-3">
                  ...
                  <h2 className="text-sm font-black text-white uppercase tracking-wider">Penanganan Hama &amp; Penyakit Spesifik</h2>
                </div>

                <div className="space-y-3">
                  {pests.map((item: any, idx: number) => (
                    <div key={idx} className="p-4 rounded-2xl bg-[#0c1410] border border-emerald-900/60 space-y-2">
              ...
              {/* Link ke Dokter AI */}
              <div className="p-4 rounded-3xl bg-[#14231b]/80 border border-emerald-800/40 flex items-center justify-between">
                <div>
                  <h3 className="text-xs font-bold text-white">Tanaman Mengalami Gejala Lain?</h3>
                  <p className="text-[10px] text-emerald-200/60">Foto spesimen &amp; dapatkan diagnosa dari Dokter AI</p>
                </div>
                <Link
                  href="/doctor"
